...
class PayNowProofUploadView(APIView):
    ...
    def post(self, request):
        ...
        if not basket_id:
            return Response({"detail": "basket_id is required"}, status=status.HTTP_400_BAD_REQUEST)
        try:
        ...
        reference = f"{settings.ORDER_PREFIX}{settings.BASE_ORDER_NUMBER + int(basket.id)}"
        cache_key = f"paynow-proof:{basket.id}"
        cache.set(cache_key, {"path": saved_path, "donation": donation, "reference": reference}, timeout=2 * 60 * 60)
...
class CheckoutEmailView(APIView):
    ...
    def post(self, request):
        ...
        if not basket_id or not email:
            return Response({"detail": "basket_id and email are required"}, status=status.HTTP_400_BAD_REQUEST)
...
class CheckoutAddressView(APIView):
    ...
    def post(self, request):
        ...
        if not basket_id or not isinstance(address, dict):
            return Response({"detail": "basket_id and address object are required"}, status=status.HTTP_400_BAD_REQUEST)
...
class PlaceOrderView(APIView):
    ...
    def post(self, request):
        ...
        if temp_key:
            ...
            if not cached:
                return Response({"detail": "Invalid or expired temp_key"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            upload = request.FILES.get("payment_proof")
            if not upload:
                return Response({"detail": "Provide either temp_key or payment_proof"}, status=status.HTTP_400_BAD_REQUEST)
            ext = os.path.splitext(upload.name)[1]
            temp_name = f"payments/temp/{uuid.uuid4()}{ext}"
        ...
        qs = DynamicShippingMethod._default_manager.filter(active=True, available_to_public=True)
        if code:
            method = qs.filter(code=code).first()
            if not method:
                return Response({"detail": "Invalid shipping_method_code"}, status=status.HTTP_400_BAD_REQUEST)
        else:
            if settings.GLOBAL_SELF_COLLECTION_REQUIRED:
                ...
                method = qs.first()
            if not method:
                return Response({"detail": "No shipping method available"}, status=status.HTTP_400_BAD_REQUEST)

        # Calculate shipping and order totals (donation added at source/event level)
        shipping_charge = method.calculate(basket)
        order_total = OrderTotalCalculator(request=request).calculate(basket, shipping_charge)
        ...
        try:
            ...
            with default_storage.open(payment_file_path, "rb") as fh:
                content = fh.read()
            order_payment_source.payment_proof.save(filename, ContentFile(content), save=False)
        except Exception:
            return Response({"detail": "Unable to read payment proof"}, status=status.HTTP_400_BAD_REQUEST)

        # Compute amount debited
        if donation is None:
            donation = 0
        total_with_donation = (order_total.incl_tax or order_total.excl_tax) + Decimal(donation)
        ...
        placer.add_payment_event("paynow-processing", total_with_donation, reference=reference)
        ...
        try:
            ...
            order = placer.place_order(
                ...
                guest_email=(request.data.get("email") or cache.get(f"guest-email:{basket.id}") or ""),
                request=request,
            )
            basket.submit()
        except Exception as e:
            return Response({"detail": f"Unable to place order: {e}"}, status=status.HTTP_400_BAD_REQUEST)
        finally:
