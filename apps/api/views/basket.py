...
class BasketCreateView(APIView):
    ...
    def post(self, request):
        ...
        return Response({"cart_id": basket.id, "basket": _serialize_basket(basket, request)}, status=status.HTTP_201_CREATED)
...
class BasketLinesView(APIView):
    ...
    def _get_basket(self, request, basket_id) -> Basket:
        ...
        if basket.owner_id and request.user.is_authenticated and basket.owner_id != request.user.id:
    ...
    def post(self, request, basket_id: str):
        ...
        try:
            ...
            return Response({"detail": f"Cannot add to basket: {e}"}, status=status.HTTP_409_CONFLICT)
...
class BasketLineDetailView(APIView):
    ...
    def _get_line(self, request, basket_id, line_id) -> Line:
        ...
        if line.basket.owner_id and request.user.is_authenticated and line.basket.owner_id != request.user.id:
            raise PermissionError("Forbidden")
...
class BasketApplyVoucherView(APIView):
    ...
    def post(self, request, basket_id: str):
        ...
        # Check availability
        available, message = voucher.is_available_to_user(user=request.user if request.user.is_authenticated else None)
        if not available:
            return Response({"detail": message or "Voucher unavailable"}, status=status.HTTP_400_BAD_REQUEST)
...
class BasketMergeView(APIView):
    ...
    def post(self, request):
        ...
        if not source_cart_id:
            return Response({"detail": "source_cart_id is required"}, status=status.HTTP_400_BAD_REQUEST)
        source = _find_guest_basket(source_cart_id)
        if not source:
            return Response({"detail": "Source basket not found"}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            return Response({"detail": f"Unable to merge baskets: {e}"}, status=status.HTTP_400_BAD_REQUEST)
