...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--template",
            type=str,
            help="Template for the email.",
            default="oscar/communication/emails/adhoc/reopen_site.html",
        )
        parser.add_argument(
            "--subject",
            type=str,
            help="Subject for the email.",
            default="The BirdSoc SG Shop is open again!",
        )
        parser.add_argument(
            "--from_email",
            type=str,
        ...
        parser.add_argument(
            "--reply_to_email", type=str, help="Reply-to email for the email."
        )
    ...
    def handle(self, *args, **options):
        ...
        for email_addr in set(
            EmailAlert._default_manager.filter(sent_date__isnull=True).values_list(
                "email", flat=True
            )
        ):
        ...
        if not options["dry_run"]:
            EmailAlert._default_manager.filter(email__in=sent).update(
                sent_date=timezone.localtime().date()
            )
